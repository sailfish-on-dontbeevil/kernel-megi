...
static void mpwr_remove_generic(struct mpwr_dev *mpwr)
{
	if (mpwr->rfkill) {
		rfkill_unregister(mpwr->rfkill);
		rfkill_destroy(mpwr->rfkill);
...
static size_t mpwr_serdev_receive_buf(struct serdev_device *serdev,
...
static void mpwr_pdev_remove(struct platform_device *pdev)
{
